...
velo_align = lambda img, gamma=1: img_color_map(img=normalize(normalize(truncate_outliers(img))**gamma)-np.mean(normalize(normalize(truncate_outliers(img))**gamma)), cmap=velo_cmap)
